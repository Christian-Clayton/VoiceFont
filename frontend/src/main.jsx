...
function App() {
  ...
  return <><a className="skip-link" href="#main">Skip to workspace</a>
    <header className="topbar"><a className="brand" href="/calibrate"><span className="brand-mark">vf</span>VoiceFont<span className="local-badge">LOCAL</span></a>
      <nav aria-label="Workspace"><div className="nav-buttons">{[['calibrate','Calibrate'],['library','Voice library'],['speech','Create speech'],['experiments','Acoustic ML']].map(([id,label]) => <button key={id} id={'nav-'+id} className={'nav-button '+(view===id?'active':'')} aria-current={view===id?'page':undefined} onClick={()=>navigate(id)}>{label}</button>)}</div></nav><span className="private-note">Your voice. On your machine.</span></header>
